...
A mobile-first PWA customer service agent powered by an LLM API (OpenAI gpt-4o by
default; Anthropic Claude also supported) with function/tool calling.
...
Live demo: https://order-assistant-et60.onrender.com (test runner at /tests.html)
...
Excel (CustomerOrders-Summer26.xlsx)
   └─► data/orders.json  (12 original + 5 new rows = 17 orders)
          └─► server.js  (zero-dependency Node server)
                 ...
                 ├─ LLM API — OpenAI gpt-4o (key server-side only, via env var)
...
```
cp .env.example .env      # paste your OPENAI_API_KEY (or ANTHROPIC_API_KEY)
...
The LLM API key lives only in the server environment. The browser talks to
`/api/chat`; it never sees the key or the upstream API.
